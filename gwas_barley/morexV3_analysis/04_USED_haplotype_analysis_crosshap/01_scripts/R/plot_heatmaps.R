...
# ---------------------------------------------------------------------------
# plot_heatmaps.R -- one heatmap PDF per gene x MGmin x epsilon (one page per
# haplotype group). Ported unchanged from the proven tool: genotypes binarised
# from the RAW per-gene VCF (0/0 -> Reference, 1/1 -> Alternate; no flip because
# the raw VCF keeps the original Morex V3 allele order), rows clustered within
# group by Hamming distance ignoring NA, columns annotated by marker group + r2.
# `title` carries the gene's origin lead SNP + significant/marginal class.
# ---------------------------------------------------------------------------
...
mk_palette <- function(n) {
  if (n <= 9) brewer.pal(max(3, n), "Set1")[seq_len(n)]
  else grDevices::colorRampPalette(brewer.pal(9, "Set1"))(n)
}

write_heatmaps_for_eps <- function(HapObject, label, gene_file, title, trait, eps, MGmin,
                                   raw_path, common_ids, vcf_raw_ids, out_pdf) {
  if (is.null(HapObject[[label]])) return(invisible(NULL))
  dir.create(dirname(out_pdf), recursive = TRUE, showWarnings = FALSE)

  hap_map <- HapObject[[label]]$Indfile %>% select(Ind, hap) %>% distinct() %>%
    mutate(hap = as.character(hap))
  var_map <- HapObject[[label]]$Varfile %>% select(ID, MGs, meanr2) %>%
  ...
  if (sum(keep_idx) < MGmin) stop("Too few variants after common-ID filtering in vcfR matrix.")
  ...
  gt_bin <- matrix(as.numeric(gt_bin), nrow = nrow(gt), ncol = ncol(gt),
                   dimnames = list(var_ids, colnames(gt)))
  m01_all <- t(gt_bin)

  in_both <- intersect(rownames(m01_all), hap_map$Ind)
  if (length(in_both) == 0) stop("No matching sample IDs between VCF and CrossHap Indfile.")
  ...
  r2_tbl <- var_map %>% filter(MarkerGroup != "Unlinked") %>% group_by(MarkerGroup) %>%
  ...
  annot_col$MarkerGroup <- vapply(annot_col$MarkerGroup, function(x) {
    if (x == "Unlinked") {
      ...
      paste0(x, " (n=", as.integer(mg_counts[[x]]), ", R2=", format(r2_map[[x]], nsmall = 2), ")")
    }
  }, character(1))
  ...
  if (length(unlinked_name) > 0) mg_colors[unlinked_name] <- "#E0E0E0"
  ann_colors <- list(MarkerGroup = mg_colors)

  group_stats <- hap_map %>% group_by(hap) %>% summarize(n_total = dplyr::n(), .groups = "drop") %>%
    arrange(desc(n_total), hap)

  cell_h_pt <- 10; extra_h_in <- 2.5
  ...
  hm_colors <- c("#FFFACD", "#2F4F4F"); hm_breaks <- c(-0.5, 0.5, 1.5)

  grDevices::pdf(out_pdf, width = 24, height = pdf_h, onefile = TRUE)
  on.exit(grDevices::dev.off(), add = TRUE)

  for (k in seq_len(nrow(group_stats))) {
    g <- group_stats$hap[k]; n_total <- group_stats$n_total[k]
    samples_g <- intersect(hap_map$Ind[hap_map$hap == g], rownames(m01_all))
    ...
    cluster_rows_arg <- if (nrow(m01_g) >= 2) hclust(pairwise_hamming_ignore_na(m01_g), method = "average") else FALSE

    main_title <- paste0(gene_clean_name, " | ", title, "\n",
                         "epsilon=", eps, " | Haplotype group: ", g, " | n_total=", n_total, "\n",
                         "Genotypes: Light Yellow = Reference | Dark Teal = Alternate | Grey = Missing (NA)")

    ph <- pheatmap::pheatmap(m01_g, color = hm_colors, breaks = hm_breaks, na_col = "grey70",
                             cluster_rows = cluster_rows_arg, cluster_cols = FALSE,
                             show_colnames = FALSE, show_rownames = TRUE,
                             annotation_col = annot_col, annotation_colors = ann_colors,
                             fontsize_row = 8, border_color = "white", main = main_title,
                             cellheight = cell_h_pt, legend = FALSE, silent = TRUE)
