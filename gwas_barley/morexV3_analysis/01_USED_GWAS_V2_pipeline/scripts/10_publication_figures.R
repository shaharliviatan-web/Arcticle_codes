...
# TAG-ready publication folder for the CHOSEN config: BLUP x 3 PCs.
...
PHENO      <- "BLUP"                              # Best Linear Unbiased Predictor
PHENO_LONG <- "BLUP (Best Linear Unbiased Predictor)"
N_PCS    <- 3L
PC_TAG   <- sprintf("pc%d", N_PCS)
CFG_LABEL <- sprintf("%s × %d PCs", PHENO, N_PCS)  # e.g. "BLUP × 3 PCs"; used in README/prose
ALPHA    <- 0.10
N_ALL    <- 7110996L
N_PRUNED <- 590462L
...
SET_NAME <- "publication_BonfOnly_BLUP_3PC"
...
readme <- c(
  sprintf("# Publication folder — %s (Bonferroni only)", CFG_LABEL),
  "",
  "## Chosen configuration",
  "",
  sprintf("- **Phenotype value**: %s", PHENO_LONG),
  ...
  sprintf("- λ_GC across the 4 traits (%s): **%.4f–%.4f** (see `tables/per_trait_summary.tsv`)", CFG_LABEL, lam_lo, lam_hi),
  ...
  sprintf("- A 24-cell sensitivity analysis (4 traits × {BLUP,BLUE} × {3,5,10} PCs) informed the configuration; %s was adopted for parsimony with adequate genomic control (λ_GC near 1 for all traits).", CFG_LABEL),
...
ct <- c(
  sprintf("# Cross-trait interpretation — %s", CFG_LABEL),
  "",
...
ct <- c(ct,
  ...
  "",
  sprintf("All four traits show slight deflation (λ_GC %.4f–%.4f, all above the 0.95 floor) under %s, consistent with adequate population-structure correction on a continuously structured wild barley sample. Deflation is mild enough that the strongest signals remain robust.", lam_lo, lam_hi, CFG_LABEL),
  "",
  "## Caveats",
  "",
