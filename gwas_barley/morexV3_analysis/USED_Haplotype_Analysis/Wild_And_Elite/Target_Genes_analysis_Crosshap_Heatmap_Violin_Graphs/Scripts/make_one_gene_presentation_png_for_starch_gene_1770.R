...
OUTPUT_DIR <- file.path(RUN_DIR, "Presentation_PNG_NO_EliteInViolinPlot")
PNG_WIDTH_IN <- 18
PNG_HEIGHT_IN <- 16
PNG_DPI <- 300
...
pick_representative_guided <- function(wild_mat, elite_mat = NULL) {
  ...
  candidate_tbl <- candidate_tbl[order(candidate_tbl$missing_n, candidate_tbl$mean_elite_distance, candidate_tbl$sample_id), , drop = FALSE]
