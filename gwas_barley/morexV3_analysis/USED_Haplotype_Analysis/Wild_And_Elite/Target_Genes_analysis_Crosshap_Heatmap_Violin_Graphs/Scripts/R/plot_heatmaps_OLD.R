...
write_heatmaps_for_target <- function(HapObject, target, merged_path, common_pos, vcf_merged_ids, out_pdf) {
  ...
  ann_colors <- list(
    Population = c(Wild = "#1b9e77", Elite = "#d95f02"),
    MarkerGroup = mg_colors
  )

  group_stats <- hap_map %>%
    mutate(Population = ifelse(grepl("^HS", Ind), "Wild", "Elite")) %>%
    group_by(hap) %>%
    summarize(
      n_total = n(),
  ...
  for (k in seq_len(nrow(group_stats))) {
    ...
    rownames(ann_g) <- rownames(m01_g)

    cluster_rows_arg <- FALSE
    if (nrow(m01_g) >= 2) {
      d <- pairwise_hamming_ignore_na(m01_g)
      cluster_rows_arg <- hclust(d, method = "average")
    }
