...
def restRequest(url):
    ...
    # Errors are indicated by HTTP status codes.
        ...
        # Local patch: requests .content is bytes; decode to str to match the
        # success path above (the upstream client crashes on 'str + bytes' when
        # a transient HTTPError is hit during status polling). Keep bytes only
        # if the body is genuinely binary (e.g. image/gzip results).
