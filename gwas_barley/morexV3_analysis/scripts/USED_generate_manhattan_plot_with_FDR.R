...
for (ps_file in ps_files) {
  ...
  for (q_val in fdr_levels) {
      ...
      output_filename <- file.path(current_plots_dir, paste0("NEW_manhattan_", phenotype_name_full, "_pc5_FDR", q_val, ".png"))
      ...
      if (length(sig_indices) > 0) {
        ...
        # CASE B: No hits — no line drawn, annotation only
        fdr_log10 <- NA
        is_theoretical <- TRUE
        label_suffix <- ": no SNPs passed"
      }
      ...
      if (!is_theoretical) {
        ...
        plot_y_limit <- ceiling(max(max_observed, bonferroni_log10, 6, na.rm = TRUE)) + 1
      }
      ...
      # Blue FDR Line (Real hits only)
      if (!is_theoretical) {
        abline(h = fdr_log10, col = "blue2", lty = 2, lwd = 1.5)
      }
      
      fdr_annotation <- if (is_theoretical) {
        paste0("FDR ", q_val, label_suffix)
      } else {
        paste0("FDR ", q_val, " = ", round(fdr_log10, 2))
      }
